...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_text'])) {
    ...
    $reply = $connection->real_escape_string(trim($_POST['reply_text']));
    $connection->query("UPDATE inquiry SET Reply='$reply' WHERE Inquiry_Id=$inqId AND Course_Id IN (SELECT Course_Id FROM course WHERE Provider_Id=$providerId)");
    header("Location: provider_inquiries.php?replied=1"); exit();
}
...
<div class="page-wrapper">
  ...
  <?php if (isset($_GET['replied'])): ?><div class="alert alert-success"><i class="fas fa-check"></i> Reply sent!</div><?php endif; ?>

  <div class="inquiry-list" style="margin-top:24px">
    <?php if ($inquiries->num_rows === 0): ?>
      <div class="empty-state" style="text-align:center;padding:60px;background:var(--white);border-radius:var(--radius-lg)">
        <i class="fas fa-inbox" style="font-size:48px;color:var(--border);display:block;margin-bottom:16px"></i>
        <h3>No inquiries yet</h3>
        <p style="color:var(--text-muted)">Student questions about your courses will appear here.</p>
      </div>
    <?php else: ?>
